...
class SkipGramWithNegativeSampling(Model):
    def __init__(self,
        ...
        self._device = f'cuda:{cuda_device}' if torch.cuda.is_available() and cuda_device >= 0 else 'cpu'
        ...
        if vocab._retained_counter:
            ...
            self._word_dist[vocab.get_token_index(token=vocab._padding_token, namespace='words')] = 0
            self._word_dist[vocab.get_token_index(token=vocab._oov_token, namespace='words')] = 0
    ...
    def forward(self,
        ...
        loss = self._loss(batch_size, context_size, pred, targets)
        self._ws353(self.vocab, self.embedder, self._device)
        output_dict = {'loss': loss}
        return output_dict

    @overrides
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        metrics = {'WS353_S': self._ws353.get_metric(reset)}
        return metrics

    def _sample(self, batch_size: int, context_size: int) -> torch.Tensor:
        # (batch_size * context_size * num_neg_samples)
        neg_samples = torch.multinomial(
            self._word_dist,
            batch_size * context_size * self._num_neg_samples,
            replacement=True)
        return neg_samples

    def _loss(self, batch_size: int, context_size: int,
             pred: torch.Tensor, targets: Dict[str, torch.Tensor]) -> torch.Tensor:
        # (batch_size * context_size)
        pos_samples = targets['words'].view(-1)
        # (batch_size * context_size * num_neg_samples)
        neg_samples = self._sample(batch_size, context_size)
        # (batch_size * context_size)
        ...
        neg_indexer = pos_indexer * self._num_neg_samples
